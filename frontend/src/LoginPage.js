...
class LoginPage extends Component {
    ...
    submit = async (e) => {
        ...
        if (window.location.host.indexOf("localhost") !== -1) {
            ...
            url = "https://api.monitoramos.com.br/login"
        }
        let xhttp = new XMLHttpRequest();
    ...
    signup = (e) => {
        ...
        if (window.location.host.indexOf("localhost") !== -1) {
            ...
            url = "https://api.monitoramos.com.br/register"
        }
    ...
    forgotPass = (e) => {
        e.preventDefault();
        const target = e.target;
        let resetEmail = target.resetEmail.value
        //const credentials = { username, password };
        ...
        if (!token) {
            ...
            xhttp.open("GET", url, true);
            xhttp.setRequestHeader('token', '@f3fg4ieWEFwfI3R3@4REFFSFEG$%dfsdf');
            xhttp.setRequestHeader('username', resetEmail);
        ...
        let url;
        if (window.location.host.indexOf("localhost") !== -1) {
            url = "http://localhost:8065/forgot-password"
        } else {
            url = "https://api.monitoramos.com.br/forgot-password"
        }
        let xhttp = new XMLHttpRequest();
        xhttp.open("POST", url, true);
        xhttp.setRequestHeader('Content-Type', 'application/json');
